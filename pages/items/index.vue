  ...
  <div>
    ...
    <div>
      ...
      >
        <template v-if="$auth.loggedIn" #[`item.actions`]="{ item }">
          <div class="d-flex gap-2">
            ...
            <v-btn
              v-if="$auth.loggedIn && $auth.user.permissions.includes('update_item')"
            ...
            <v-btn
              v-if="$auth.loggedIn && $auth.user.permissions.includes('delete_item')"
              class="error--text"
              elevation="0"
              small
      ...
      <div class="text-center pt-8">
        <v-pagination v-if="meta.last_page > 1" v-model="page" :length="meta.last_page" />
      </div>
    </div>
    ...
    <v-snackbar v-model="snackbar" :timeout="8000" :color="snackbarColor">
      ...
      <template #action="{ attrs }">
        <v-btn class="v-btn--active" text v-bind="attrs" @click="snackbar = false">
          {{ $t("close") }}
        </v-btn>
      </template>
    </v-snackbar>
      ...
      if (value) {
        ...
        this.items = this.coreItems.filter((item) => {
          if (value.station_id && !value.from && !value.to) {
            ...
            return (
              item.station.id === value.station_id.id &&
              item.date >= value.from &&
              item.date <= value.to
            );
          } else if (!value.station_id && value.from && value.to) {
